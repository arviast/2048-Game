...
//
//
void prepend(Node *&pHead, int board[], int score, int move, int squaresPerSide) {
    Node *pTemp = new Node;
    pTemp->gameScore = score;
    pTemp->moveNumber = move;
    
    for(int i = 0; i < (squaresPerSide*squaresPerSide); i++) {
        pTemp->board[i] = board[i];
        std::cout << pTemp->board[i];
    }
    pTemp->pNext = pHead;
    pHead = pTemp;
}
...
//
//
void restoreMove( Node *pHead, int board[], int& score, int& move, int squaresPerSide) {
    Node *pTemp = pHead;
    
    for(int i = 0; i < (squaresPerSide*squaresPerSide); i++) {
        board[i] = pTemp->board[i];
    }
    score = pTemp->gameScore;
    move = pTemp->moveNumber;
}
...
int main()
{
    ...
    // ...
    prepend( pHead, board, score, move, squaresPerSide);
    ...
    while( true)
    {
        ...
        copyBoard( previousBoard, board, squaresPerSide);  // copy from board into previousBoard
        ...
        switch (userInput) {
            ...
            case 'u':
                undoMove(pHead);
                restoreMove( pHead, board, score, move, squaresPerSide);
                continue;
            default:
